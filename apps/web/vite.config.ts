	...
	mergeConfig(sharedTestConfig, {
		...
		test: {
			...
			coverage: {
				...
				thresholds: {
					// One aggregate floor rather than one per directory under src/lib.
					// Coverage there is deliberately uneven — presentational registry
					// components carry far less than the hooks and chat components — and a
					// per-directory floor would only be met by writing tests that assert
					// how something looks, or by deleting code that earns its place. The
					// aggregate holds the app to the documented standard while leaving that

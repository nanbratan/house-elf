		...
		test: {
			...
			coverage: {
				...
				exclude: [
					...
					// The HTML document shell: `<html>`, `<head>`, `HeadContent` and
					// `Scripts`. Framework wiring with no logic of its own, and jsdom
					// cannot meaningfully render a document element. What it is for —
					// that the page arrives styled and hydrated — is only provable in a
					// real browser, so it is checked there instead.
					'src/routes/__root.tsx',

					// Route files are pure wiring: a path bound to a component that is
